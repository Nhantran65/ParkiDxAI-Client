import { ICreateHospitalData, ICreateUserData, IUpdateUserData, UserRole } from 'booking-doctor-js';
import { useTranslation } from 'react-i18next';
import * as Yup from 'yup';

const useCreateHospitalSchema = () => {
    const { t } = useTranslation(['bkd.hospital']);

    const schema: Yup.ObjectSchema<ICreateHospitalData> = Yup.object({
...
export default useCreateHospitalSchema;
