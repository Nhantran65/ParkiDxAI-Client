import { ICreateStatusData, ICreateUserData, IUpdateUserData, UserRole } from 'booking-doctor-js';
import { useTranslation } from 'react-i18next';
import * as Yup from 'yup';

const useCreateStatusSchema = () => {
    const { t } = useTranslation(['bkd.status']);

    const schema: Yup.ObjectSchema<ICreateStatusData> = Yup.object({
...
export default useCreateStatusSchema;
